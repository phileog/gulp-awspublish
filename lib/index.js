...
function toAwsParams(file) {
  ...
  // params.ContentLength = file.ContentLength || 0;
...
Publisher.prototype.publish = function (headers, options) {
  ...
  return through.obj(function (file, enc, cb) {
    ...
    if (file.isStream()) {
      ...
      file.s3.state = 'create'; // force create

      // upload instead of putObject...
      _this.client.upload(toAwsParams(file), function(err, data) {
        if (err) return cb(err);

        file.s3.date = new Date();
        file.s3.etag = data.ETag; // bof ?
        cb(err, file);
      });
    }
